...
if [ "$hipchatuserexists" = "" ]; then
	...
	hcp=`echo "$hipchatpass1" | sed -n 2p`
	#count the length of said password
	hcplength=`echo ${#hcp}`
	...
	info=`"$CD" bubble --title "All done!" --text "Please login to hipchat, your username is: $username@pingidentity.com" --icon-file /Library/Application\ Support/JAMF/EndUserSupport/PI_SQLogo.icns`
